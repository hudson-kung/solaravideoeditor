...
export default function SignInPage() {
  const [name,setName]=useState(""); const [email,setEmail]=useState(""); const [returnTo,setReturnTo]=useState("/dashboard");
  useEffect(()=>{ const saved=localStorage.getItem("solara-profile"); if(saved){try{const p=JSON.parse(saved);setName(p.name||"");setEmail(p.email||"")}catch{}} const target=new URLSearchParams(location.search).get("returnTo");if(target?.startsWith("/"))setReturnTo(target)},[]);
  const submit=(e:FormEvent)=>{e.preventDefault();localStorage.setItem("solara-profile",JSON.stringify({name:name.trim(),email:email.trim(),createdAt:new Date().toISOString()}));location.href=returnTo};
